...
const Registration = () => {
  ...
  const result2 = selectedValue / price;
  const mjcstackamountValue = mjcstackamount / price;
  ...
  const approveTokens = async () => {
    try {
      ...
      let spender = "0x2849897F5f7D3a1088Ecae416CC6fBEf3Dd05D37"; //contract address
  ...
  const buyToken = async () => {
    ...
    try {
      ...
      const data = await buyTokens({
        args: [
          "0x2849897F5f7D3a1088Ecae416CC6fBEf3Dd05D37",
          usdtAmt.toString(),
          selectedValue,
        ],
      });
